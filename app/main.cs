...
namespace app
{
    public partial class main : Form
    {
        ...
        {
            string text = String.Join(" ", db.LoadInfo("Group_Department").ToArray());
            string dir = "C:\\Data";
            string file = Path.Combine(dir, "Group_Department.txt");

            if (!Directory.Exists(dir))
                Directory.CreateDirectory(dir);

            File.WriteAllText(file, text);
            string text1 = String.Join(" ", db.LoadInfo("Surname").ToArray());
            string dir1 = "C:\\Data";
            string file1 = Path.Combine(dir1, "Surname.txt");
